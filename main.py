...
def log_broker_order(s: Signal, order_side: str, qty: int, status_text: str):
    ...
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        now(), TRADING_MODE, s.strategy_id.upper(), s.symbol.upper(),
        s.signal.upper(), order_side, qty, float(s.price), s.exchange.upper(),
        s.product.upper(), s.order_type.upper(), s.validity.upper(),
...
def broker_action(s: Signal, order_side: str, qty: int):
    ...
    if TRADING_MODE == "LIVE":
        payload = build_broker_payload(s, order_side, qty)
        kotak = KotakClient()
        result = kotak.place_order(payload)
        status_text = result.get("status", "LIVE_UNKNOWN")
        log_broker_order(s, order_side, qty, status_text)
...
def webhook(s: Signal):
    ...
    c = conn()
    try:
        c.execute("INSERT INTO alerts(alert_hash,time,raw) VALUES(?,?,?)", (alert_hash, now(), raw))
        c.commit()
    except Exception:
        c.close()
        return {"status": "ignored", "reason": "Duplicate alert blocked"}
    closed_today = c.execute("SELECT * FROM trades WHERE trade_date=? AND status='CLOSED'", (today(),)).fetchall()
    ...
    if signal in ["EXIT", "CLOSE"]:
        ...
        c.execute("""
        UPDATE trades SET exit=?, status='CLOSED', pnl=?, exit_reason=? WHERE id=?
...
def dashboard(strategy_filter: str = "ALL", user: str = Depends(authenticate)):
    ...
    if selected_strategy == "ALL":
        ...
        trades = c.execute("SELECT * FROM trades WHERE strategy_id=? ORDER BY id DESC", (selected_strategy,)).fetchall()
    c.close()
    ...
    for sid, st in sorted(strategy_stats.items()):
        ...
        strategy_cards += f"""
        <div class="card"><h3>Strategy {sid}</h3><h2 style="color:{color};">{round(st['pnl'], 2)}</h2><p>Closed: {st['trades']}</p><p>Open: {st['open']}</p><p>Win Rate: {wr}%</p></div>
        """
    daily_rows = ""
    for d, st in sorted(daily_stats.items(), reverse=True):
        ...
        color = "green" if st["pnl"] > 0 else "red" if st["pnl"] < 0 else "black"
        daily_rows += f"""
        <tr><td>{d}</td><td>{st['trades']}</td><td>{st['wins']}</td><td>{st['losses']}</td><td>{wr}%</td><td style="color:{color};font-weight:bold;">{round(st['pnl'],2)}</td></tr>
        """
    rows = ""
    for t in trades:
        pnl_color = "green" if t["pnl"] > 0 else "red" if t["pnl"] < 0 else "black"
        rows += f"""
        <tr><td>{t['id']}</td><td>{t['time']}</td><td>{t['strategy_id']}</td><td>{t['symbol']}</td><td>{t['trade_type']}</td><td>{t['side']}</td><td>{t['entry']}</td><td>{t['exit']}</td><td>{t['qty']}</td><td>{t['status']}</td><td style="color:{pnl_color};font-weight:bold;">{round(t['pnl'], 2)}</td><td>{t['exit_reason']}</td></tr>
        """
    broker_rows = ""
    for b in broker_orders:
        broker_rows += f"""
        <tr><td>{b['id']}</td><td>{b['time']}</td><td>{b['mode']}</td><td>{b['strategy_id']}</td><td>{b['symbol']}</td><td>{b['signal']}</td><td>{b['order_side']}</td><td>{b['qty']}</td><td>{b['price']}</td><td>{b['exchange']}</td><td>{b['product']}</td><td>{b['order_type']}</td><td>{b['status']}</td></tr>
        """
    return f"""
    ...
    .danger {{ background:#c0392b; }} .download {{ background:#2980b9; }} .oi {{ background:#16a085; }}
    ...
    th,td {{ padding:8px; border:1px solid #ddd; text-align:center; }} th {{ background:#111; color:white; }}
    ...
    <div class="topbar">
      ...
      <a class="btn oi" href="/oi">NSE OI Page</a>
      <a class="btn download" href="/export_csv">Download CSV</a>
      <a class="btn danger" href="/reset">Reset All Trades</a>
      <form method="get" action="/dashboard"><label><b>Filter Strategy:</b></label><select name="strategy_filter">{strategy_options}</select><button type="submit">Apply</button></form>
    </div>
    <div class="cards">
    ...
    <h2>Strategy Performance</h2><div class="cards">{strategy_cards}</div>
    <h2>Trade Log</h2><table><tr><th>ID</th><th>Time</th><th>Strategy</th><th>Symbol</th><th>Type</th><th>Side</th><th>Entry</th><th>Exit</th><th>Qty</th><th>Status</th><th>P&L</th><th>Exit Reason</th></tr>{rows}</table>
    ...
    <h2>Daily Report</h2><table><tr><th>Date</th><th>Trades</th><th>Wins</th><th>Losses</th><th>Win Rate</th><th>P&L</th></tr>{daily_rows}</table>
    </body></html>
    """
...
def oi_page(user: str = Depends(authenticate)):
    ...
    return f"""
    <html><head><title>NSE OI Dashboard</title><style>
    body {{ font-family: Arial; background:#f4f6f8; padding:20px; }} h1 {{ color:#111; }}
    .topbar {{ display:flex; gap:10px; margin-bottom:15px; flex-wrap:wrap; }}
    .btn {{ padding:10px 15px; background:#111; color:white; border-radius:6px; text-decoration:none; }} .refresh {{ background:#16a085; }}
    table {{ width:100%; border-collapse:collapse; background:white; margin-top:20px; font-size:14px; }}
    th,td {{ padding:10px; border:1px solid #ddd; text-align:center; }} th {{ background:#111; color:white; }}
    .note {{ background:#fff3cd; padding:10px; border-radius:6px; margin-bottom:15px; font-weight:bold; }}
    </style></head><body>
    <h1>NSE OI Dashboard</h1>
    <div class="note">This page is separate from trading dashboard. If NSE blocks Railway/server, status error will show here.</div>
    <div class="topbar"><a class="btn" href="/dashboard">Back to Dashboard</a><a class="btn refresh" href="/oi">Refresh OI</a></div>
    <table><tr><th>Symbol</th><th>CE Total OI</th><th>PE Total OI</th><th>PCR</th><th>Max CE OI Strike</th><th>Max PE OI Strike</th><th>Sentiment</th></tr>{rows}</table>
    </body></html>
    """
