...
@app.get("/")
def home():
    return {"status": "Algo Dashboard Running", "mode": TRADING_MODE, "dashboard": "/dashboard", "webhook": "/webhook"}
...
def dashboard(strategy_filter: str = "ALL", user: str = Depends(authenticate)):
    ...
    for oi in oi_data:
        if "error" in oi:
            oi_rows += f"""<tr><td>{oi['symbol']}</td><td colspan="6">{oi['error']}</td></tr>"""
        else:
            oi_rows += f"""<tr><td>{oi['symbol']}</td><td>{oi['ce_total']}</td><td>{oi['pe_total']}</td><td>{oi['pcr']}</td><td>{oi['max_ce']}</td><td>{oi['max_pe']}</td><td>{oi['sentiment']}</td></tr>"""
    ...
    <body>
        ...
        <table><tr><th>Symbol</th><th>CE OI</th><th>PE OI</th><th>PCR</th><th>Max CE Strike</th><th>Max PE Strike</th><th>Sentiment</th></tr>{oi_rows}</table>
        <div class="cards">
        ...
        <h2>Trade Log</h2>
        <table><tr><th>ID</th><th>Time</th><th>Strategy</th><th>Symbol</th><th>Type</th><th>Side</th><th>Entry</th><th>Exit</th><th>Qty</th><th>Status</th><th>P&L</th><th>Exit Reason</th></tr>{rows}</table>
        <h2>Broker Order Test Log</h2>
        <table><tr><th>ID</th><th>Time</th><th>Mode</th><th>Strategy</th><th>Symbol</th><th>Signal</th><th>Order Side</th><th>Qty</th><th>Price</th><th>Exchange</th><th>Product</th><th>Order Type</th><th>Status</th></tr>{broker_rows}</table>
        <h2>Daily Report</h2>
        <table><tr><th>Date</th><th>Trades</th><th>Wins</th><th>Losses</th><th>Win Rate</th><th>P&L</th></tr>{daily_rows}</table>
    </body>
    </html>
    """
